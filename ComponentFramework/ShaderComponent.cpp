...
ShaderComponent::ShaderComponent(Component* parent_, const char* vsFilename_, const char* fsFilename_):Component(parent_), shaderID(0),vertShaderID(0),fragShaderID(0) {
...
bool ShaderComponent::CompileAttach(){
    GLint status;
	try { 		
		const char* vsText = ReadTextFile(vsFilename); // read and load contents of file
		const char* fsText = ReadTextFile(fsFilename); // read and load contents of file
		if (vsText == nullptr || fsText == nullptr) {
			return false;
		}
		/// GL_VERTEX_SHADER and GL_FRAGMENT_SHADER are defined in glew.h
		vertShaderID = glCreateShader(GL_VERTEX_SHADER); // create a vertex shader
		fragShaderID = glCreateShader(GL_FRAGMENT_SHADER); // create a fragment shader
		/// Check for errors
		...
		glShaderSource(vertShaderID, 1, &vsText, 0); // get address of the pointer of the array, how many lines, where to start?   
		glShaderSource(fragShaderID, 1, &fsText, 0); // get address of the pointer of the array, how many lines, where to start? 
    
		glCompileShader(vertShaderID);
		/// Check for errors
		glGetShaderiv(vertShaderID, GL_COMPILE_STATUS, &status); // get shader int and pointer name, status, pointer to 'status' variable
		if (status == 0) {
			GLsizei errorLogSize = 0; // set error message to be 0
			GLsizei titleLength;
			std::string errorLog = vsFilename; // create error message
			errorLog += "\nVERT:\n"; // add to the vert log
			titleLength = errorLog.length();
			glGetShaderiv(vertShaderID, GL_INFO_LOG_LENGTH, &errorLogSize); // get int and ptr info from vert shader, error log length and size
			errorLog.resize(titleLength + errorLogSize); // resize error log to be length of title
		...
		shaderID = glCreateProgram(); // create id of the shader
		glAttachShader(shaderID, fragShaderID); // attach frag shader to shader ID
		glAttachShader(shaderID, vertShaderID); // attach frag shader to shader ID
		if(vsText) delete[] vsText; // delete
		if(fsText) delete[] fsText; // delete

	}catch(std::string error){ // catch the exceptoion
		printf("%s\n",&error[0]); // write in the file there was an error
		Debug::Error(error, __FILE__, __LINE__); // input to console menu
		return false; // die off
	}
	return true;
}
...
bool ShaderComponent::Link(){
	GLint status;
	try{
			glLinkProgram(shaderID); // link the shader ID
			/// Check for errors
